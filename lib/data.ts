...
// Single-row operations for the per-item admin tour routes (create/edit/
// delete one tour). Using these instead of the bulk saveTours()/full-resave
// pattern avoids two real risks: an UPDATE that changes a tour's id would
// leave an orphaned duplicate row behind (saveTours only ever INSERTs/
// UPDATEs by id, it never renames a row), and resaving every tour on every
// single edit/delete does needless writes and risks a timeout as the table
// grows. saveTours() is kept for the initial DB seed in scripts/setup-db.mjs.
export async function insertTour(t: TourRecord): Promise<void> {
  ...
  await sql`
    INSERT INTO tours (
      ...
      ${JSON.stringify(t.includes || [])}::jsonb, ${t.duration || null}, ${t.rating},
      ${t.reviews}, ${t.price}, ${t.originalPrice ?? null}, ${t.image}, ${t.imageAlt},
      ${t.hrefPath}, ${t.hrefExtra || null}, ${!!t.featured}, ${t.bestFor}, ${t.priceTableColumn1 || ""}, ${t.priceTableFeature || ""}, ${count}
    )
  `;
}
